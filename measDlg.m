function Answer=measDlg(Prompt, Title, NumLines, DefAns, Resize, types, DefVals)
...
PmInfo=EdInfo;
PmInfo.Style           = 'popupmenu';

CbInfo=EdInfo;
% CbInfo.FontWeight      = get(InputFig,'DefaultUicontrolFontWeight');
CbInfo.Style           = 'checkbox';
CbInfo.BackgroundColor = FigColor;
    ...
    switch types{lp}
        ...
        case {'file', 'folder'}
            EditHandle(lp)=uicontrol(InputFig    , ...
                'Style', 'pushbutton', ...
                'Units', 'pixels', ...
                'Position'   ,[ TxtXOffset EditYOffset(lp)  EditHeight(lp) EditHeight(lp) ], ...
                'String'     ,''           , ...
                'CData', imread('folder28.bmp'), ...
                'Tag'        ,'Filebtn', ...
             'Callback', {@pbOpenDir_Callback});
            fileIconIdx = fileIconIdx + 1;
            fileIcon2ET(fileIconIdx) = [EditHandle(lp)];
            EditHandle(lp)=uicontrol(InputFig    , ...
                EdInfo      , ...
                'Max'        ,NumLines(lp,1)       , ...
                'Position'   ,[ TxtXOffset+25 EditYOffset(lp) TxtWidth-30 EditHeight(lp) ], ...
                'String'     ,DefAns{lp}           , ...
                'Tag'        ,'Edit');
%             fileIcon2ET(1, 2) = [EditHandle(lp)];            
        otherwise
            delete(InputFig);
            errordlg(['types must be: edit, popupmenu or checkbox. Not :' types{lp}]);
          return
    end
...
if ~multicolumn
  for lp = 1:length(Data.Edit)
    ...
    EditPos(3) = FigWidth - round(Data.TextMargin/2) - EditPos(1);
...
function pbOpenDir_Callback(object, eventdata) %#ok<*DEFNU,INUSD>
ind = 0;
for i = 1 : length(fileIcon2ET)
    if fileIcon2ET(i) == object
        ind = i;
        break;
    end
end
...
if strcmp(types{ind}, 'file')
    [file path] = uigetfile(DefVals{ind});
    if ( file ~= 0 )
        set( EditHandle(ind), 'String', [ path  file]);
    end
elseif strcmp(types{ind}, 'folder')
    dir = uigetdir(DefVals{ind});
    if ( dir ~= 0 )
        set( EditHandle(ind), 'String',dir);
    end
end
