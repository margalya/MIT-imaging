classdef Fit2DGaussianWings < FitTypes
%Fit2DGaussianWings - fit a 2D Guassian only to the external part of the cloud,
% in order to determine the temperature of Fermi-Dirac distributions.
% To avoid big error in the number of atoms, the C coefficient (background) is still set by the regular 2DGaussian fit

   properties ( Constant = true )
       ID = 'Fit2DGaussianWings';
   end
...
%            [pic x0 y0] = appData.data.plots{appData.data.plotType}.getAnalysisPic(appData);  % the whole pic
           ...
           f = fittype('OD*exp(-(x-x0)^2/2/sigmaX^2 - (y-y0)^2/2/sigmaY^2) + C', 'coefficients', {'OD', 'x0', 'y0', 'sigmaX', 'sigmaY', 'C'},...
            ...
            if ( numel(pic) > 450^2 )
                ...
                [obj.res, obj.gof, ~] = fit([X, Y], binnedPic, f);
            else
...
%             disp(['Mean 2D Gaussian fit resiuals RMS = ' num2str(sqrt(mean(obj.output.residuals(:).^2))) ])
%             figure;
%             imagesc(LowPassFilter(reshape(obj.output.residuals,size(pic)),30,9) )
% %             imagesc(reshape(obj.output.residuals,size(pic)) )
%             colorbar
...
%             disp(['Un-calibrated background photon count: ' num2str(test)])
            % last
            appData.data.fits{appData.consts.fitTypes.twoDGaussianWings} = obj;
%            appData = appData.data.plots{appData.consts.plotTypes.ROI}.setPic(appData, pic);
       end
...
%            disp(obj.C)
           ...
           if isempty(obj.conf) %backwards compatability - before using fit in 2DGaussian
               ...
               tof = 4; %TOF in ms
               text( 0, -0.05,  ['Far field temperature (TOF = ' num2str(tof) ' ms) = ' num2str( round(singleShotTemperature(appData, obj, tof)*1e6)) ' uK'], 'fontsize', 12, 'Parent', appData.ui.pFitResults);
           end
           end
   end
end
